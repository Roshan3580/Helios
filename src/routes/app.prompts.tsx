  ...
  return (
    <div>
      ...
      <div className="border border-rule bg-card">
        ...
        {PROMPTS.map((p) => (
          <div key={p.name} className="grid grid-cols-12 items-center gap-3 border-b border-rule px-4 py-3">
            ...
            <div className="col-span-2"><StatusBadge tone={p.score > 85 ? "success" : p.score > 80 ? "warn" : "danger"}>{p.score}%</StatusBadge></div>
            <div className="col-span-1 text-right font-mono text-[11px] text-muted-foreground">{p.lat}</div>
            <div className="col-span-1 text-right font-mono text-[11px] text-muted-foreground">{p.cost}</div>
...
+ If the user mentions API keys or tokens, prefer "security".
  Always cite the source document.`}</pre>
        ...
        <div className="col-span-12 lg:col-span-5 border border-rule bg-card">
          <div className="border-b border-rule px-4 py-2.5"><Eyebrow>Score history</Eyebrow></div>
          <ul className="divide-y divide-rule">
            {[["v6","88.1%","success"],["v5","82.4%","warn"],["v4","79.0%","warn"],["v3","74.2%","danger"]].map(([v,s,t]) => (
